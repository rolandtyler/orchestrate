...
func TestUpdateChildren_Execute(t *testing.T) {
	...
	t.Run("should execute use case successfully if parentJobUUID is set", func(t *testing.T) {
		...
		jobsToUpdate[0].Status = utils.StatusPending
		jobsToUpdate[1].Status = utils.StatusPending

		mockJobDA.EXPECT().Search(ctx, &entities.JobFilters{ParentJobUUID: parentJobUUID, Status: utils.StatusPending},
			tenants).Return(jobsToUpdate, nil)
